...
namespace
{
    ...
    const std::string kTracePassFilename = "RenderPasses/ReSTIRPT/TracePass.cs.slang";
    const std::string kReflectTypesFile = "RenderPasses/ReSTIRPT/ReflectTypes.cs.slang";
    const std::string kSpatialReusePassFile = "RenderPasses/ReSTIRPT/SpatialReuse.cs.slang";
    const std::string kTemporalReusePassFile = "RenderPasses/ReSTIRPT/TemporalReuse.cs.slang";
    const std::string kSpatialPathRetraceFile = "RenderPasses/ReSTIRPT/SpatialPathRetrace.cs.slang";
    const std::string kTemporalPathRetraceFile = "RenderPasses/ReSTIRPT/TemporalPathRetrace.cs.slang";
    const std::string kComputePathReuseMISWeightsFile = "RenderPasses/ReSTIRPT/ComputePathReuseMISWeights.cs.slang";
...
{
    lib.registerClass("ReSTIRPT", kDesc, ReSTIRPT::create);
    ScriptBindings::registerBinding(ReSTIRPT::registerBindings);
}


void ReSTIRPT::updateDict(const Dictionary& dict)
{
    // cleanToDefaultValue
...
void ReSTIRPT::registerBindings(pybind11::module& m)
{
    ...
    pybind11::class_<ReSTIRPT, RenderPass, ReSTIRPT::SharedPtr> pass(m, "ReSTIRPT");
...
ReSTIRPT::SharedPtr ReSTIRPT::create(RenderContext* pRenderContext, const Dictionary& dict)
{
    return SharedPtr(new ReSTIRPT(dict));
}

ReSTIRPT::ReSTIRPT(const Dictionary& dict)
{
...
bool ReSTIRPT::parseDictionary(const Dictionary& dict)
{
    ...
    for (const auto& [key, value] : dict)
    {
        ...
        else logWarning("Unknown field '" + key + "' in ReSTIRPT dictionary");
    }
...
Dictionary ReSTIRPT::getSpecializedScriptingDictionary()
{
    Dictionary d;
...
void ReSTIRPT::compile(RenderContext* pContext, const CompileData& compileData)
{
...
void ReSTIRPT::setScene(RenderContext* pRenderContext, const Scene::SharedPtr& pScene)
{
    mpScene = pScene;
    mParams.frameCount = 0;
...
Texture::SharedPtr ReSTIRPT::createNeighborOffsetTexture(uint32_t sampleCount)
{
...
bool ReSTIRPT::renderStatsUI(Gui::Widgets& widget)
{
    bool dirty = false;
...
void ReSTIRPT::preparePathTracer(const RenderData& renderData)
{
...
void ReSTIRPT::setShaderData(const ShaderVar& var, const RenderData& renderData, bool isPathTracer, bool isPathGenerator) const
{
...
void ReSTIRPT::generatePaths(RenderContext* pRenderContext, const RenderData& renderData, int sampleId)
{
...
void ReSTIRPT::tracePass(RenderContext* pRenderContext, const RenderData& renderData, const ComputePass::SharedPtr& pass, const std::string& passName, int sampleID)
{
    PROFILE(passName);
...
void ReSTIRPT::PathReusePass(RenderContext* pRenderContext, uint32_t restir_i, const RenderData& renderData, bool isTemporalReuse, int spatialRoundId, bool isLastRound)
{
...
void ReSTIRPT::PathRetracePass(RenderContext* pRenderContext, uint32_t restir_i, const RenderData& renderData, bool temporalReuse /*= false*/, int spatialRoundId /*= 0*/)
{
...
Program::DefineList ReSTIRPT::StaticParams::getDefines(const ReSTIRPT& owner) const
{
